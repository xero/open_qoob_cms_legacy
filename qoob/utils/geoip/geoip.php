<?php
/**
 * geo ip look up functions
 * GeoIP provides a non-invasive way to determine geographical information 
 * about their visitors in real-time buy utilizing precompiled lookup databases.
 *
...
"Aland Islands","Guernsey","Isle of Man","Jersey","Saint Barthelemy","Saint Martin"
  ...
  "EU", "NA", "NA"
    ...
    public function getCountry($ip) {
      if(strpos($ip, ":") > 0) {
        $gi = geoip_open(dirname(__FILE__).SLASH."GeoIPv6.dat", GEOIP_STANDARD);
        $country = geoip_country_name_by_addr_v6($gi, $ip);
      } else {
        $gi = geoip_open(dirname(__FILE__).SLASH."GeoIP.dat", GEOIP_STANDARD);
