...
<div id="adminForm">
	<form name="addGalleryImage" method="post" action="<?=QOOB_DOMAIN; ?>backdoor/modGalleryImg/" enctype="multipart/form-data">
		<div class="row">
			<div class="lbl">File: </div>
			<div class="inputs">
				<a href="<?=BASE_URL;?>style/img/projects/<?=$theFile; ?>"><?=$theFile; ?></a>
			</div>
		</div>
		<div class="row">
...
?>
		...
		<div class="row">
			<div class="lbl"></div>
			<div class="inputs">
				<input type="submit" name="btnMod" title=" Modify Image " value=" Modify Image " class="btnForm" onclick="return modVerify()"/>
			</div>
		</div>
		<input type="hidden" name="theRealURL" id="theRealURL" value="<?=$txtURL; ?>" />
		<input type="hidden" name="theFile" id="theFile" value="<?=$theFile; ?>" />
